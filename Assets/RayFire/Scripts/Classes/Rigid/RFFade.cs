...
namespace RayFire
{
    ...
    public class RFFade
    {
        ...
        // Start life coroutine
        IEnumerator LivingCor (RayfireRigidRoot scrRoot, RFShard shard)
        {
            /*
            
            // Wait for simulation get rest
            if (scrRoot.fading.lifeType == RFFadeLifeType.BySimulationAndLifeTime)
                yield return scrRoot.StartCoroutine(SimulationCor (scr));
            
            */
            
            // Get final life duration
            float lifeDuration = scrRoot.fading.lifeTime;
            if (scrRoot.fading.lifeVariation > 0)
                lifeDuration += Random.Range (0f, scrRoot.fading.lifeVariation);
            ...
            // Exclude from simulation and keep object in scene
            if (scrRoot.fading.fadeType == FadeType.SimExclude)
                FadeExclude (scrRoot, shard);

            /*
            
            // Exclude from simulation, move under ground, destroy
            else if (scrRoot.fading.fadeType == FadeType.MoveDown)
                scrRoot.StartCoroutine (FadeMoveDown (scr));

            // Start scale down and destroy
            else if (scrRoot.fading.fadeType == FadeType.ScaleDown)
                scrRoot.StartCoroutine (FadeScaleDownCor (scr));

            // Destroy object
            else if (scrRoot.fading.fadeType == FadeType.Destroy)
                RayfireMan.DestroyFragment (scr, scr.rootParent);
        ...
        static void FadeExclude (RayfireRigid scr)
        {
            ...
            {
                UnityEngine.Object.Destroy (scr.physics.rigidBody);
                UnityEngine.Object.Destroy (scr.physics.meshCollider);
        ...
        // Exclude from simulation and keep object in scene
        static void FadeExclude (RayfireRigidRoot scrRoot, RFShard shard)
        {
            // Not going to be reused
            if (scrRoot.reset.action == RFReset.PostDemolitionType.DestroyWithDelay)
            {
            ...
            // Going to be reused 
            else if (scrRoot.reset.action == RFReset.PostDemolitionType.DeactivateToReset)
            {
...
/*
        // Exclude from simulation, move under ground, destroy
        static IEnumerator FadeMoveDown (RayfireRigidRoot scrRoot)
        {
            // Activate inactive
            if (scr.simulationType == SimType.Inactive)
                scr.Activate();

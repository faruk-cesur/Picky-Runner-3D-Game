...
namespace RayFire
{
    ...
    public static class RFFragment
    {
        ...
        // Filter out meshes by size
        static void RemoveBySize (ref Mesh[] meshes, ref Vector3[] pivots, ref List<RFDictionary> origSubMeshIdsRf, RayfireShatter scrShatter)
        {
            if (scrShatter.advanced.absoluteSize > 0 || scrShatter.advanced.relativeSize > 0)
            {
                List<Mesh>         newMeshes = new List<Mesh>();
                List<Vector3>      newPivots = new List<Vector3>();
                List<RFDictionary> newIds    = new List<RFDictionary>();
                float              size      = scrShatter.meshRenderer.bounds.size.magnitude * scrShatter.advanced.relativeSize / 100f;
     
                for (int i = 0; i < meshes.Length; i++)
                {
                    if (scrShatter.advanced.absoluteSize > 0)
                        if (meshes[i].bounds.size.magnitude > scrShatter.advanced.absoluteSize)
                        {
                            newMeshes.Add (meshes[i]);
                            newPivots.Add (pivots[i]);
                            newIds.Add (origSubMeshIdsRf[i]);
                    ...
                    if (scrShatter.advanced.relativeSize > 0)
        ...
        {
            // Voro Simple mode: for rigig without shatter
            if (scrShatter == null)
                return 1;

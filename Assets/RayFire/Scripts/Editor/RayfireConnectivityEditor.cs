    ...
    public class RayfireConnectivityEditor : Editor
    {
        ...
        static void ClusterDraw(RayfireConnectivity targ)
        {
            if (targ.showNodes == true || targ.showConnections == true)
            {
                ...
                {
                    // Reinit connections
                    // RayfireConnectivity.InitCachedShardsByRigidList (targ.rigidList, targ.cluster);
                    ...
                    for (int i = 0; i < targ.cluster.shards.Count; i++)
                    {
                        if (targ.cluster.shards[i].tm != null)
                        {
                            ...
                            if (targ.showConnections == true)
                            {
                                ...
                                // Preview
                                foreach (RFShard neibShard in targ.cluster.shards[i].neibShards)
                                    if (neibShard.tm != null)
                                        Gizmos.DrawLine (targ.cluster.shards[i].tm.position, neibShard.tm.position);
                            }
                        }
                    }
                }
